...
uint8_t W5100Class::init(void)
{
	...
	initSS();
	resetSS();
	SPI_X.beginTransaction(SPI_ETHERNET_SETTINGS);
...
uint16_t W5100Class::write(uint16_t addr, const uint8_t *buf, uint16_t len)
{
	...
	if (chip == 51) {
		for (uint16_t i=0; i<len; i++) {
			setSS();
			SPI_X.transfer(0xF0);
			SPI_X.transfer(addr >> 8);
			SPI_X.transfer(addr & 0xFF);
			addr++;
			SPI_X.transfer(buf[i]);
			resetSS();
		}
	} else if (chip == 52) {
		setSS();
		...
		SPI_X.transfer(cmd, 4);/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

#ifdef SPI_HAS_TRANSFER_BUF
		SPI_X.transfer(buf, NULL, len);        //////////////-------------------------------------------------------
#elif defined(USE_SPI_DMA)
		SPI_X.spiSend(buf,len);  //Nitrof
#else
		...
		if (len <= 5) {
			...
			SPI_X.transfer(cmd, len + 3);
		} else {
			SPI_X.transfer(cmd, 3); //////////////////////////////////////////////////////////////////////////////////////////
#ifdef SPI_HAS_TRANSFER_BUF
			SPI_X.transfer(buf, NULL, len);//////////////////////////////////////////////////////////////////////////////////////////
#elif defined(USE_SPI_DMA)
		SPI_X.spiSend(buf,len);  //Nitrof
...
uint16_t W5100Class::read(uint16_t addr, uint8_t *buf, uint16_t len)
{
	...
	if (chip == 51) {
		...
		memset(buf, 0, len);
#ifdef USE_SPI_DMA
		SPI_X.spiRec(buf,len);  //Nitrof
#else		
		SPI_X.transfer(buf, len);         ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#endif
		resetSS();
	} else { // chip == 55
		setSS();
